...
func example1_BasicConversion() {
	...
	setProp(obj, "DPI", 300)
	// setProp(obj, "DPIWidth", 300)   // (Optional)
	// setProp(obj, "DPIHeight", 300)  // (Optional)
	setProp(obj, "OutWithTTF", 1)
	setProp(obj, "EmbeddedSearchText", 0)
	setProp(obj, "zoomtype", 0)
	setProp(obj, "OutputByLayoutSize", 0)
	setProp(obj, "nBackgroundColor", 7) // White background
...
func example2_OnePdfPerView() {
	...
	callMethod(obj, "SetOutColor", 1, 5) // Red -> Blue
...
func example3_BatchConversion() {
	...
	// for _, dwgFile := range dwgFiles {
	//     callMethod(obj, "AddFile", filepath.Join(inputDir, dwgFile))
	// }
	// setProp(obj, "Width", 1024)
	// setProp(obj, "Height", 768)
	// setProp(obj, "ColorMode", 1)
	// callMethod(obj, "ConvertFiles", filepath.Join(outputDir, "batch_output.pdf"))
...
func example4_Advanced() {
	...
	callMethod(obj, "SetOutputScale", float32(1.0))
...
// ============================================================
func example5_DWFtoDWG() {
	inputFile := `D:\input\dwf\drawing.dwf`
	outputFile := `D:\output\dwg\output.dwg`
	outVersion := int16(24)
	...
	fmt.Println("Converting DWF to DWG...")
	fmt.Println("  Input: ", inputFile)
	fmt.Println("  Output:", outputFile)

	callMethod(obj, "ConvertDWFtoDWGDXF", inputFile, outputFile, outVersion)
	fmt.Println("Done:", outputFile)
}
