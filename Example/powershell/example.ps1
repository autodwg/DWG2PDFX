...
function Convert-Basic {
    ...
    # $obj.DPIWidth = 300;  $obj.DPIHeight = 300  # (Optional) Separate DPI
    ...
    try {
        ...
        Write-Host "Conversion failed. (Error: $_)" -ForegroundColor Red
    }
}
...
function Convert-Batch {
    ...
    # foreach ($file in $dwgFiles) { $obj.AddFile($file.FullName) }
    # $obj.Width = 1024;  $obj.Height = 768;  $obj.ColorMode = 1
    # $obj.ConvertFiles("$DstDir\batch_output.pdf")
